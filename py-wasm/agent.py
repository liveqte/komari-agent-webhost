#!/usr/bin/env python3

#
# komari-agent-python-1.1.0-wasm by liming2038
# 基础原则：不是有贡献就可以对普通用户指指点点，普通用户使用开源项目也有平等发声权力，不能打着维护原作者的旗号来贬低他人，远离饭圈文化。
# 在平等、互相尊重基础上的使用本项目。
#

import asyncio
import json
import os
import platform
import sys
import time
import socket
import aiohttp
import websockets
from aiohttp import web
from datetime import datetime, timezone
from typing import Dict, List, Optional, Any, Tuple
from urllib.parse import urlparse
from websockets.exceptions import ConnectionClosed, ConnectionClosedError, ConnectionClosedOK
import shutil

def parse_env_args() -> Dict[str, Any]:
    """解析环境变量"""
    return {
        'http_server': os.getenv('KOMARI_HTTP_SERVER', ''),
        'token': os.getenv('KOMARI_TOKEN', ''),
        'interval': float(os.getenv('KOMARI_INTERVAL', '5.0')),
        'reconnect_interval': int(os.getenv('KOMARI_RECONNECT_INTERVAL', '10')),
        'ignore_unsafe_cert': os.getenv('KOMARI_IGNORE_UNSAFE_CERT', 'true').lower() != 'false',
        'log_level': int(os.getenv('KOMARI_LOG_LEVEL', '0')),
        'disable_remote_control': os.getenv('KOMARI_DISABLE_REMOTE_CONTROL', 'false').lower() == 'true'
    }
# ============================================================================
# WASM 环境检测
# ============================================================================
def is_wasm_environment() -> bool:
    """检测是否在 WASM/WASI 环境中运行"""
    # 方法 1: 检查平台
    if sys.platform == 'wasi':
        return True
    # 方法 2: 检查机器架构
    if platform.machine() in ('wasm32', 'wasm64'):
        return True
    # 方法 3: 检查 Pyodide
    if hasattr(sys, '_emscripten_info'):
        return True
    # 方法 4: 检查环境变量
    if os.environ.get('WASM_RUNTIME'):
        return True
    return False

IS_WASM = is_wasm_environment()

# ============================================================================
# 条件导入：WASM 不支持的模块
# ============================================================================
if not IS_WASM:
    import psutil
    import pty
    import fcntl
    import termios
    import select
    import subprocess
    import signal
    PSUTIL_AVAILABLE = True
else:
    psutil = None
    pty = None
    fcntl = None
    termios = None
    select = None
    subprocess = None
    signal = None
    PSUTIL_AVAILABLE = False

# ============================================================================
# Logger (保持不变)
# ============================================================================
class Logger:
    """日志处理器"""
    _log_level = 0
    
    @classmethod
    def set_log_level(cls, level: int):
        cls._log_level = level
    
    @classmethod
    def _log(cls, message: str, level: str = "INFO"):
        if cls._log_level == 0 and level != "ERROR":
            return
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_message = f"[{timestamp}] [{level}] {message}"
        print(log_message)
        if level == "ERROR":
            print(log_message, file=sys.stderr)
    
    @classmethod
    def debug(cls, message: str, debug_level: int = 1):
        if cls._log_level == debug_level:
            cls._log(message, "DEBUG")
    
    @classmethod
    def info(cls, message: str):
        cls._log(message, "INFO")
    
    @classmethod
    def warning(cls, message: str):
        cls._log(message, "WARNING")
    
    @classmethod
    def error(cls, message: str):
        cls._log(message, "ERROR")
# ============================================================================
#  AgentStatus (探针运行状态共享容器)
# ============================================================================        
class AgentStatus:
    """探针运行状态共享容器"""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        self.is_running = False
        self.start_time: Optional[datetime] = None
        self.last_heartbeat: Optional[str] = None
        self.last_basic_info: Optional[Dict[str, Any]] = None
        self.last_realtime_info: Optional[Dict[str, Any]] = None
        self.error_count = 0
        self.last_error: Optional[str] = None
        self.websocket_connected = False
        self.http_server_runner = None  # 存储 aiohttp AppRunner（改名避免混淆）
        self.config = config or {}  # ✅ 修复：初始化 config
        
    def set_config(self, config: Dict[str, Any]):
        """设置配置（如果在创建时未传入）"""
        self.config = config
        
    def update_heartbeat(self):
        """更新心跳时间"""
        self.last_heartbeat = datetime.now(timezone.utc).isoformat()
        
    def update_basic_info(self, info: Dict[str, Any]):
        """更新基础信息缓存"""
        self.last_basic_info = info.copy()
        
    def update_realtime_info(self, info: Dict[str, Any]):
        """更新实时监控缓存"""
        self.last_realtime_info = info.copy()
        
    def update_websocket_status(self, connected: bool):
        """✅ 新增：更新 WebSocket 连接状态"""
        self.websocket_connected = connected
        if connected:
            self.update_heartbeat()  # 连接成功时更新心跳
        
    def record_error(self, error: Exception):
        """记录错误"""
        self.error_count += 1
        self.last_error = f"{type(error).__name__}: {str(error)}"
        
    def reset_error_count(self):
        """✅ 新增：重置错误计数（连接恢复后调用）"""
        self.error_count = 0
        self.last_error = None
        
    def to_dict(self) -> Dict[str, Any]:
        """导出状态为字典"""
        uptime = None
        if self.start_time:
            uptime = (datetime.now(timezone.utc) - self.start_time).total_seconds()
        
        # ✅ 修复：安全访问 config
        http_server_url = self.config.get('http_server') if self.config else None
        
        return {
            "agent": {
                "version": SystemInfoCollector.VERSION,
                "runtime": "wasm" if IS_WASM else "native",
                "is_running": self.is_running,
                "start_time": self.start_time.isoformat() if self.start_time else None,
                "uptime_seconds": round(uptime, 2) if uptime else None,
                "last_heartbeat": self.last_heartbeat,
            },
            "connection": {
                "websocket_connected": self.websocket_connected,
                "http_server": http_server_url,  # ✅ 修复：使用本地变量
            },
            "health": {
                "error_count": self.error_count,
                "last_error": self.last_error,
                "status": "healthy" if self.error_count < 10 else "degraded" if self.error_count < 50 else "unhealthy"
            },
            "cache": {
                "basic_info": self.last_basic_info,
                "realtime_info": self.last_realtime_info,
            }
        }
# ============================================================================
# WASM 兼容的系统信息收集器
# ============================================================================
class SystemInfoCollector:
    """系统信息收集器 - WASM 兼容版本"""
    VERSION = "komari-agent-python-1.1.0-wasm"
    
    def __init__(self):
        self.last_network_stats = {'rx': 0, 'tx': 0}
        self.total_network_up = 0
        self.total_network_down = 0
        self.last_network_time = time.time()
        self._cpu_initialized = False
        self._cpu_init_lock = asyncio.Lock()
        # WASM 环境下的模拟数据
        self._wasm_cpu_cores = 4
        self._wasm_mem_total = 512 * 1024 * 1024  # 512MB
        self._wasm_disk_total = 1024 * 1024 * 1024  # 1GB
        self._script_start_time = time.time()
    
    async def get_basic_info(self) -> Dict[str, Any]:
        """获取基础系统信息"""
        dist_info = self._get_linux_distribution()
        ipv4, ipv6 = await asyncio.gather(
            self._get_public_ip_v4(),
            self._get_public_ip_v6(),
            return_exceptions=True
        )
        ipv4 = ipv4 if not isinstance(ipv4, Exception) else None
        ipv6 = ipv6 if not isinstance(ipv6, Exception) else None
        
        os_name = f"{dist_info['name']} {dist_info['version']}" if dist_info['name'] != 'Unknown' else platform.system()
        
        info = {
            "arch": platform.machine(),
            "cpu_cores": self._get_cpu_cores(),
            "cpu_name": self._get_cpu_name(),
            "disk_total": await self._get_disk_total(),
            "gpu_name": "",
            "ipv4": ipv4,
            "ipv6": ipv6,
            "mem_total": self._get_mem_total(),
            "os": os_name,
            "kernel_version": platform.release(),
            "swap_total": self._get_swap_total(),
            "version": self.VERSION,
            "virtualization": self._get_virtualization()
        }
        Logger.debug(f"基础信息数据：{json.dumps(info, indent=2)}", 1)
        return info
    
    async def get_realtime_info(self) -> Dict[str, Any]:
        """获取实时监控信息"""
        cpu_usage = await self._get_cpu_usage()
        network_stats = await self._get_network_stats()
        memory_info = await self._get_memory_info()
        disk_info = await self._get_disk_info()
        
        process_count = self._get_process_count()
        
        info = {
            "cpu": {"usage": cpu_usage},
            "ram": {
                "total": memory_info["ram_total"],
                "used": memory_info["ram_used"]
            },
            "swap": {
                "total": memory_info["swap_total"],
                "used": memory_info["swap_used"]
            },
            "load": self._get_load_avg(),
            "disk": {
                "total": disk_info["total"],
                "used": disk_info["used"]
            },
            "network": {
                "up": network_stats["up"],
                "down": network_stats["down"],
                "totalUp": network_stats["total_up"],
                "totalDown": network_stats["total_down"]
            },
            "connections": {
                "tcp": await self._get_tcp_connections(),
                "udp": await self._get_udp_connections()
            },
            "uptime": self._get_uptime(),
            "process": process_count,
            "message": "WASM" if IS_WASM else ""
        }
        Logger.debug(f"实时监控数据：{json.dumps(info, indent=2)}", 2)
        return info
    
    # ========== CPU 相关 ==========
    def _get_cpu_cores(self) -> int:
        """获取 CPU 核心数"""
        if PSUTIL_AVAILABLE:
            try:
                return psutil.cpu_count() or 1
            except Exception:
                pass
        # WASM 降级：尝试从环境变量或返回默认值
        try:
            return int(os.environ.get('WASM_CPU_CORES', 4))
        except ValueError:
            return 4
    
    def _get_cpu_name(self) -> str:
        """获取 CPU 名称"""
        if IS_WASM:
            return "WASM Virtual CPU"
        try:
            if platform.system() == "Windows":
                import winreg
                key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 
                                   r"HARDWARE\DESCRIPTION\System\CentralProcessor\0")
                cpu_name = winreg.QueryValueEx(key, "ProcessorNameString")[0]
                winreg.CloseKey(key)
                return cpu_name.strip()
            else:
                with open('/proc/cpuinfo', 'r') as f:
                    for line in f:
                        if line.strip().startswith('model name'):
                            return line.split(':')[1].strip()
        except Exception as e:
            Logger.debug(f"获取 CPU 名称失败：{e}", 1)
        return "Unknown CPU"
    
    async def _get_cpu_usage(self) -> float:
        """获取 CPU 使用率"""
        if PSUTIL_AVAILABLE:
            async with self._cpu_init_lock:
                if not self._cpu_initialized:
                    try:
                        psutil.cpu_percent(interval=0.1)
                        self._cpu_initialized = True
                        return 0.0
                    except Exception:
                        pass
                try:
                    usage = psutil.cpu_percent(interval=None)
                    return round(max(0, min(100, usage)), 2)
                except Exception:
                    pass
        # WASM 降级：返回模拟数据
        return self._simulate_cpu_usage()
    
    def _simulate_cpu_usage(self) -> float:
        """WASM 环境下模拟 CPU 使用率"""
        import random
        return round(random.uniform(5.0, 25.0), 2)
    
    # ========== 内存相关 ==========
    def _get_mem_total(self) -> int:
        """获取内存总量"""
        if PSUTIL_AVAILABLE:
            try:
                return psutil.virtual_memory().total
            except Exception:
                pass
        return self._wasm_mem_total
    
    def _get_swap_total(self) -> int:
        """获取 Swap 总量"""
        if PSUTIL_AVAILABLE:
            try:
                return psutil.swap_memory().total
            except Exception:
                pass
        return 0  # WASM 通常无 swap
    
    async def _get_memory_info(self) -> Dict[str, int]:
        """获取内存信息"""
        if PSUTIL_AVAILABLE:
            try:
                virtual_memory = psutil.virtual_memory()
                swap_memory = psutil.swap_memory()
                return {
                    "ram_total": virtual_memory.total,
                    "ram_used": virtual_memory.used,
                    "swap_total": swap_memory.total,
                    "swap_used": swap_memory.used
                }
            except Exception as e:
                Logger.debug(f"获取内存信息失败：{e}", 2)
        # WASM 降级：模拟数据
        used = int(self._wasm_mem_total * 0.6)  # 假设 60% 使用率
        return {
            "ram_total": self._wasm_mem_total,
            "ram_used": used,
            "swap_total": 0,
            "swap_used": 0
        }
    
    # ========== 磁盘相关 ==========
    async def _get_disk_info(self) -> Dict[str, int]:
        """获取磁盘信息"""
        if PSUTIL_AVAILABLE:
            try:
                total_bytes = 0
                used_bytes = 0
                seen_physical_devices = set()
                partitions = psutil.disk_partitions(all=True)
                for partition in partitions:
                    device = partition.device
                    mountpoint = partition.mountpoint
                    fstype = partition.fstype
                    if fstype in {'tmpfs', 'devtmpfs', 'overlay', 'squashfs', 
                                 'proc', 'sysfs', 'debugfs', 'configfs', 
                                 'cgroup', 'cgroup2', 'pstore', 'bpf', 
                                 'tracefs', 'securityfs', 'efivarfs'}:
                        continue
                    physical_device = self._get_physical_disk_device(device)
                    if not physical_device or physical_device in seen_physical_devices:
                        continue
                    if not self._is_physical_disk(physical_device):
                        continue
                    try:
                        usage = psutil.disk_usage(mountpoint)
                        total_bytes += usage.total
                        used_bytes += usage.used
                        seen_physical_devices.add(physical_device)
                    except (PermissionError, OSError):
                        continue
                return {"total": total_bytes, "used": used_bytes}
            except Exception as e:
                Logger.debug(f"获取磁盘信息失败：{e}", 5)
        # WASM 降级：返回模拟数据
        return {
            "total": self._wasm_disk_total,
            "used": int(self._wasm_disk_total * 0.3)
        }
    
    async def _get_disk_total(self) -> int:
        """获取磁盘总容量"""
        disk_info = await self._get_disk_info()
        return disk_info["total"]
    
    def _get_physical_disk_device(self, device_path: str) -> Optional[str]:
        """获取物理磁盘设备名"""
        if not PSUTIL_AVAILABLE or platform.system() != "Linux":
            return device_path
        import re
        dev_name = device_path.replace("/dev/", "")
        if not dev_name:
            return None
        if re.match(r'^[a-zA-Z0-9\.\-_]+:', dev_name) or dev_name.startswith('//'):
            return device_path
        DEVICE_PATTERNS = [
            r'^(md[0-9]+)$', r'^(sd[a-z]+)\d*$', r'^(vd[a-z]+)\d*$',
            r'^(xvd[a-z]+)\d*$', r'^(mmcblk\d+)p?\d*$', r'^(nvme\d+n\d+)p?\d*$',
        ]
        for pattern in DEVICE_PATTERNS:
            m = re.match(pattern, dev_name)
            if m:
                return f"/dev/{m.group(1)}"
        return device_path
    
    def _is_physical_disk(self, device: str) -> bool:
        """判断是否为物理磁盘"""
        if not PSUTIL_AVAILABLE:
            return False
        if platform.system() == "Windows":
            return any(device.lower().startswith(drive) for drive in ['c:', 'd:', 'e:', 'f:', 'g:', 'h:'])
        else:
            import re
            if re.match(r'^[a-zA-Z0-9\.\-_]+:', device) or device.startswith('//'):
                return True
            physical_patterns = [
                r'^/dev/sd[a-z]+$', r'^/dev/vd[a-z]+$', r'^/dev/xvd[a-z]+$',
                r'^/dev/nvme[0-9]+n[0-9]+$', r'^/dev/mmcblk[0-9]+$',
                r'^/dev/md[0-9]+$', r'^zroot/.*$',
            ]
            return any(re.match(pattern, device) for pattern in physical_patterns)
    
    # ========== 网络相关 ==========
    async def _get_network_stats(self) -> Dict[str, int]:
        """获取网络统计"""
        if PSUTIL_AVAILABLE:
            try:
                net_io = psutil.net_io_counters(pernic=True)
                current_time = time.time()
                total_current_rx = 0
                total_current_tx = 0
                exclude_patterns = ['lo', 'docker', 'veth', 'br-', 'tun', 'virbr']
                for interface, stats in net_io.items():
                    if any(pattern in interface for pattern in exclude_patterns):
                        continue
                    total_current_rx += stats.bytes_recv
                    total_current_tx += stats.bytes_sent
                
                if self.last_network_stats['rx'] == 0:
                    self.total_network_down = total_current_rx
                    self.total_network_up = total_current_tx
                    self.last_network_stats = {'rx': total_current_rx, 'tx': total_current_tx}
                    self.last_network_time = current_time
                    return {"up": 0, "down": 0, "total_up": self.total_network_up, "total_down": self.total_network_down}
                
                time_diff = current_time - self.last_network_time
                if time_diff > 0:
                    down_speed = (total_current_rx - self.last_network_stats['rx']) / time_diff
                    up_speed = (total_current_tx - self.last_network_stats['tx']) / time_diff
                    self.total_network_down = total_current_rx
                    self.total_network_up = total_current_tx
                    self.last_network_stats = {'rx': total_current_rx, 'tx': total_current_tx}
                    self.last_network_time = current_time
                    return {
                        "up": int(max(0, up_speed)),
                        "down": int(max(0, down_speed)),
                        "total_up": self.total_network_up,
                        "total_down": self.total_network_down
                    }
            except Exception as e:
                Logger.debug(f"psutil 网络统计失败：{e}", 4)
        # WASM 降级：返回模拟数据
        return self._simulate_network_stats()
    
    def _simulate_network_stats(self) -> Dict[str, int]:
        """WASM 环境下模拟网络统计"""
        import random
        current_time = time.time()
        if self.last_network_time == 0:
            self.last_network_time = current_time
        time_diff = current_time - self.last_network_time
        if time_diff > 0:
            self.total_network_down += random.randint(1000, 10000)
            self.total_network_up += random.randint(500, 5000)
            self.last_network_time = current_time
        return {
            "up": random.randint(100, 1000),
            "down": random.randint(500, 5000),
            "total_up": self.total_network_up,
            "total_down": self.total_network_down
        }
    
    async def _get_tcp_connections(self) -> int:
        """获取 TCP 连接数"""
        if PSUTIL_AVAILABLE:
            try:
                if platform.system() == "Windows":
                    result = subprocess.run(['netstat', '-n', '-p', 'tcp'],
                                          capture_output=True, text=True, timeout=5)
                    return len([line for line in result.stdout.split('\n') if 'ESTABLISHED' in line])
                else:
                    connections = psutil.net_connections(kind='tcp')
                    return len([conn for conn in connections if conn.status == 'ESTABLISHED'])
            except Exception as e:
                Logger.debug(f"获取 TCP 连接数失败：{e}", 2)
        # WASM 降级：返回当前 WebSocket 连接数（模拟）
        return 1 if IS_WASM else 0
    
    async def _get_udp_connections(self) -> int:
        """获取 UDP 连接数"""
        if PSUTIL_AVAILABLE:
            try:
                if platform.system() == "Windows":
                    result = subprocess.run(['netstat', '-n', '-p', 'udp'],
                                          capture_output=True, text=True, timeout=5)
                    return len([line for line in result.stdout.split('\n') if 'UDP' in line and line.strip()])
                else:
                    connections = psutil.net_connections(kind='udp')
                    return len(connections)
            except Exception as e:
                Logger.debug(f"获取 UDP 连接数失败：{e}", 2)
        # WASM 降级：返回 0
        return 0
    
    # ========== 系统负载 ==========
    def _get_load_avg(self) -> Dict[str, float]:
        """获取系统负载"""
        if PSUTIL_AVAILABLE:
            try:
                if hasattr(psutil, 'getloadavg') and psutil.getloadavg():
                    load = psutil.getloadavg()
                    return {
                        "load1": round(load[0], 2),
                        "load5": round(load[1], 2),
                        "load15": round(load[2], 2)
                    }
            except Exception:
                pass
        # WASM 降级：返回模拟数据
        import random
        return {
            "load1": round(random.uniform(0.1, 1.0), 2),
            "load5": round(random.uniform(0.1, 1.0), 2),
            "load15": round(random.uniform(0.1, 1.0), 2)
        }
    
    def _get_process_count(self) -> int:
        """获取进程数"""
        if PSUTIL_AVAILABLE:
            try:
                return len(psutil.pids())
            except Exception:
                pass
        # WASM 降级：返回 1（当前进程）
        return 1
    
    def _get_uptime(self) -> int:
        """获取运行时间（秒）
        
        - 原生环境：优先使用系统启动时间（psutil.boot_time）
        - WASM/降级：使用脚本启动时间（self._script_start_time）
        """
        # 方案 A: 原生环境 + psutil 可用 → 使用系统启动时间
        if PSUTIL_AVAILABLE:
            try:
                return int(time.time() - psutil.boot_time())
            except Exception:
                # psutil 调用失败，降级到方案 B
                pass
        
        # 方案 B: WASM 环境 或 psutil 不可用 → 使用脚本启动时间
        if hasattr(self, '_script_start_time') and self._script_start_time:
            return int(time.time() - self._script_start_time)
        
        # 方案 C: 兜底返回 0
        return 0
        
    # ========== 其他方法 ==========
    def _get_linux_distribution(self) -> Dict[str, str]:
        """获取 Linux 发行版信息"""
        try:
            if platform.system() == "Linux":
                if os.path.exists('/etc/os-release'):
                    with open('/etc/os-release', 'r') as f:
                        content = f.read()
                    name = 'Unknown'
                    version = 'Unknown'
                    for line in content.split('\n'):
                        if line.startswith('ID='):
                            name = line.replace('ID=', '').replace('"', '').strip()
                        elif line.startswith('VERSION_ID='):
                            version = line.replace('VERSION_ID=', '').replace('"', '').strip()
                    return {'name': name, 'version': version}
        except Exception:
            pass
        return {'name': 'WASM' if IS_WASM else 'Unknown', 'version': 'Unknown'}
    
    def _get_virtualization(self) -> str:
        """获取虚拟化信息"""
        if IS_WASM:
            return 'WASM'
        try:
            if platform.system() == "Linux":
                if os.path.exists('/.dockerenv'):
                    return 'Docker'
                if os.path.exists('/proc/1/cgroup'):
                    with open('/proc/1/cgroup', 'r') as f:
                        content = f.read()
                    if 'docker' in content:
                        return 'Docker'
                    elif 'lxc' in content:
                        return 'LXC'
                if os.path.exists('/proc/cpuinfo'):
                    with open('/proc/cpuinfo', 'r') as f:
                        content = f.read()
                    if 'QEMU' in content or 'KVM' in content:
                        return 'QEMU'
        except Exception:
            pass
        return 'None'
    
    async def _get_public_ip_v4(self) -> Optional[str]:
        """获取公网 IPv4 地址"""
        services = ['https://api.ipify.org', 'https://icanhazip.com',
                   'https://checkip.amazonaws.com', 'https://ifconfig.me/ip']
        for service in services:
            try:
                ip = await self._fetch_ip(service)
                if ip and self._is_valid_ipv4(ip):
                    return ip
            except Exception:
                continue
        return None
    
    async def _get_public_ip_v6(self) -> Optional[str]:
        """获取公网 IPv6 地址"""
        services = ['https://api6.ipify.org', 'https://icanhazip.com']
        for service in services:
            try:
                ip = await self._fetch_ip(service)
                if ip and self._is_valid_ipv6(ip):
                    return ip
            except Exception:
                continue
        return None
    
    async def _fetch_ip(self, url: str) -> str:
        """获取 IP 地址"""
        timeout = aiohttp.ClientTimeout(total=5)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, headers={'User-Agent': self.VERSION}) as response:
                if response.status == 200:
                    return (await response.text()).strip()
                else:
                    raise Exception(f"HTTP {response.status}")
    
    def _is_valid_ipv4(self, ip: str) -> bool:
        """验证 IPv4 地址"""
        try:
            socket.inet_pton(socket.AF_INET, ip)
            return True
        except socket.error:
            return False
    
    def _is_valid_ipv6(self, ip: str) -> bool:
        """验证 IPv6 地址"""
        try:
            socket.inet_pton(socket.AF_INET6, ip)
            return True
        except socket.error:
            return False

# ============================================================================
# WASM 兼容的终端会话处理器
# ============================================================================
class TerminalSessionHandler:
    """终端会话处理器 - WASM 兼容版本"""
    
    def __init__(self):
        self.heartbeat_timeout = None
        self.last_heartbeat = 0
        self.HEARTBEAT_TIMEOUT = 30
        self.process = None
        self.master_fd = None
        self.slave_fd = None
    
    async def cleanup(self):
        """清理终端资源"""
        Logger.info("执行终端资源清理...")
        if self.process and not IS_WASM:
            try:
                if hasattr(os, 'killpg') and hasattr(signal, 'SIGTERM'):
                    os.killpg(os.getpgid(self.process.pid), signal.SIGTERM)
                else:
                    self.process.terminate()
                try:
                    await asyncio.wait_for(self.process.wait(), timeout=2.0)
                except asyncio.TimeoutError:
                    self.process.kill()
            except Exception as e:
                Logger.debug(f"清理进程失败：{e}")
            self.process = None
        
        if not IS_WASM:
            for fd_name in ['master_fd', 'slave_fd']:
                fd = getattr(self, fd_name)
                if fd is not None:
                    try:
                        os.close(fd)
                    except Exception as e:
                        Logger.debug(f"关闭 {fd_name} 失败：{e}")
                    setattr(self, fd_name, None)
    
    async def start_session(self, request_id: str, server: str, token: str):
        """启动终端会话"""
        log = lambda msg: Logger.info(f"[终端会话 {request_id}] {msg}")
        
        if IS_WASM:
            log("⚠️ WASM 环境不支持 PTY 终端，返回模拟响应")
            # WASM 环境：返回错误信息或模拟终端
            return
        
        log("启动终端会话")
        try:
            terminal_url = server.replace('http', 'ws') + f"/api/clients/terminal?token={token}&id={request_id}"
            log(f"连接终端 WebSocket: {terminal_url}")
            async with websockets.connect(terminal_url) as websocket:
                log("终端 WebSocket 连接成功")
                await self._run_terminal(websocket, request_id, log)
        except ConnectionClosed:
            log(f"[终端会话] 客户端连接已断开")
        except Exception as e:
            log(f"终端会话异常：{e}")
        finally:
            await self.cleanup()
            log(f"[终端会话] 资源清理完毕：{request_id}")
    
    async def _run_terminal(self, websocket, request_id: str, log):
        """运行终端"""
        if IS_WASM:
            log("WASM 环境无法启动 PTY 终端")
            return
        
        self.master_fd = None
        self.slave_fd = None
        try:
            self.master_fd, self.slave_fd = pty.openpty()
            shell = os.environ.get('SHELL', '/bin/bash')
            self.process = await asyncio.create_subprocess_exec(
                shell,
                stdin=self.slave_fd,
                stdout=self.slave_fd,
                stderr=self.slave_fd,
                preexec_fn=os.setsid if hasattr(os, 'setsid') else None
            )
            log(f"终端进程已启动 (PID: {self.process.pid})")
            
            if self.slave_fd is not None:
                os.close(self.slave_fd)
                self.slave_fd = None
            
            fl = fcntl.fcntl(self.master_fd, fcntl.F_GETFL)
            fcntl.fcntl(self.master_fd, fcntl.F_SETFL, fl | os.O_NONBLOCK)
            
            tasks = [
                asyncio.create_task(self._handle_pty_output(websocket, self.master_fd, log)),
                asyncio.create_task(self._handle_websocket_input(websocket, self.master_fd, log)),
                asyncio.create_task(self._monitor_process(websocket, self.process, log))
            ]
            done, pending = await asyncio.wait(tasks, return_when=asyncio.FIRST_COMPLETED)
            for t in pending:
                t.cancel()
        except Exception as e:
            log(f"启动终端失败：{str(e)}")
            await self.cleanup()
    
    async def _handle_pty_output(self, websocket, master, log):
        """处理 PTY 输出到 WebSocket"""
        if IS_WASM:
            return
        try:
            while True:
                if master is None:
                    break
                rlist, _, _ = select.select([master], [], [], 0.1)
                if master in rlist:
                    data = os.read(master, 4096)
                    if not data:
                        break
                    await websocket.send(data)
                else:
                    await asyncio.sleep(0.01)
        except (OSError, ConnectionClosed):
            pass
        except Exception as e:
            log(f"处理 PTY 输出异常：{e}")
    
    async def _handle_websocket_input(self, websocket, master, log):
        """处理 WebSocket 输入到 PTY"""
        if IS_WASM:
            return
        try:
            async for message in websocket:
                if isinstance(message, bytes):
                    os.write(master, message)
                elif isinstance(message, str):
                    try:
                        data = json.loads(message)
                        if data.get('type') == 'input' and 'data' in data:
                            import base64
                            input_data = base64.b64decode(data['data']).decode()
                            os.write(master, input_data.encode())
                    except json.JSONDecodeError:
                        os.write(master, message.encode())
        except Exception as e:
            log(f"处理 WebSocket 输入异常：{e}")
    
    async def _monitor_process(self, websocket, process, log):
        """监控进程状态"""
        if IS_WASM:
            return
        try:
            await process.wait()
            log(f"终端进程退出，代码：{process.returncode}")
        except Exception as e:
            log(f"监控进程异常：{e}")

# ============================================================================
# 事件处理器
# ============================================================================
class EventHandler:
    """事件处理器"""
    
    def __init__(self, config: Dict[str, Any], disable_remote_control: bool = False):
        self.config = config
        self.disable_remote_control = disable_remote_control or IS_WASM  # WASM 默认禁用远程控制
        self.session = aiohttp.ClientSession()
    
    async def handle_event(self, event: Dict[str, Any]):
        """处理事件"""
        message_type = event.get('message', '')
        Logger.info(f"收到服务器事件：{message_type}")
        Logger.debug(f"事件详情：{json.dumps(event, indent=2)}", 2)
        
        if message_type == 'exec':
            await self._handle_remote_exec(event)
        elif message_type == 'ping':
            await self._handle_ping_task(event)
        elif message_type == 'terminal':
            await self._handle_terminal(event)
        else:
            Logger.warning(f"未知的事件类型：{message_type}")
    
    async def _handle_remote_exec(self, event: Dict[str, Any]):
        """处理远程执行"""
        if self.disable_remote_control:
            Logger.warning("远程执行功能已被禁用，忽略任务")
            return
        
        task_id = event.get('task_id', '')
        command = event.get('command', '')
        if not task_id or not command:
            Logger.error("远程执行任务缺少必要参数")
            return
        
        if self._is_dangerous_command(command):
            Logger.warning(f"检测到可能危险的命令，拒绝执行：{command}")
            await self._report_exec_result(task_id, "命令被拒绝执行：安全检查未通过", -3)
            return
        
        Logger.info(f"执行远程命令：{command}")
        await self._execute_command(task_id, command)
    
    def _is_dangerous_command(self, command: str) -> bool:
        """检查危险命令"""
        dangerous_patterns = ['rm -rf /', 'dd if=', ':(){ :|:& };:', 'reboot', 'poweroff']
        command_lower = command.lower()
        return any(pattern in command_lower for pattern in dangerous_patterns)
    
    async def _execute_command(self, task_id: str, command: str):
        """执行命令"""
        if IS_WASM:
            Logger.warning("WASM 环境不支持 subprocess，无法执行命令")
            await self._report_exec_result(task_id, "WASM 环境不支持命令执行", -1)
            return
        
        try:
            start_time = time.time()
            if platform.system() == "Windows":
                shell_cmd = ["powershell", "-Command", command]
            else:
                shell_cmd = ["sh", "-c", command]
            
            Logger.info(f"执行命令：{' '.join(shell_cmd)}")
            process = await asyncio.create_subprocess_exec(
                *shell_cmd,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                stdin=asyncio.subprocess.PIPE
            )
            
            try:
                stdout, stderr = await asyncio.wait_for(
                    process.communicate(),
                    timeout=30.0
                )
                execution_time = time.time() - start_time
                output = ""
                if stdout:
                    output += stdout.decode('utf-8', errors='replace')
                if stderr:
                    if output:
                        output += "\n=== STDERR ===\n"
                    output += stderr.decode('utf-8', errors='replace')
                exit_code = process.returncode
                Logger.info(f"命令执行完成，耗时：{execution_time:.2f}s, 退出码：{exit_code}")
                
                if len(output) > 10000:
                    output = output[:10000] + f"\n... (输出被截断，总长度：{len(output)} 字符)"
                if not output:
                    output += "无输出结果"
                await self._report_exec_result(task_id, output, exit_code)
            except asyncio.TimeoutError:
                Logger.warning(f"命令执行超时，强制终止进程")
                try:
                    process.terminate()
                    await asyncio.sleep(1)
                    if process.returncode is None:
                        process.kill()
                except:
                    pass
                await self._report_exec_result(task_id, "命令执行超时（30 秒）", -2)
        except Exception as e:
            Logger.error(f"命令执行异常：{e}")
            await self._report_exec_result(task_id, f"命令执行异常：{e}", -1)
    
    async def _report_exec_result(self, task_id: str, result: str, exit_code: int):
        """上报执行结果"""
        report_url = f"{self.config['http_server']}/api/clients/task/result?token={self.config['token']}"
        finished_at = datetime.now(timezone.utc).isoformat()
        report_data = {
            "task_id": task_id,
            "result": result,
            "exit_code": exit_code,
            "finished_at": finished_at
        }
        Logger.debug(f"上报执行结果：{json.dumps(report_data, indent=2)}", 2)
        try:
            async with self.session.post(report_url, json=report_data) as response:
                if response.status in (200, 201):
                    Logger.info("执行结果上报成功")
                else:
                    error_body = await response.text()
                    Logger.error(f"执行结果上报失败 - HTTP: {response.status}, 响应：{error_body}")
        except Exception as e:
            Logger.error(f"执行结果上报异常：{e}")
    
    async def _handle_ping_task(self, event: Dict[str, Any]):
        """处理网络探测任务"""
        task_id = event.get('ping_task_id', '')
        ping_type = event.get('ping_type', '')
        target = event.get('ping_target', '')
        if not task_id or not ping_type or not target:
            Logger.error("网络探测任务缺少必要参数")
            return
        Logger.info(f"执行网络探测：{ping_type} -> {target}")
        await self._execute_ping(task_id, ping_type, target)
    
    async def _execute_ping(self, task_id: str, ping_type: str, target: str):
        """执行网络探测"""
        try:
            latency = -1
            if ping_type == 'icmp':
                latency = await self._ping_icmp(target)
            elif ping_type == 'tcp':
                latency = await self._ping_tcp(target)
            elif ping_type == 'http':
                latency = await self._ping_http(target)
            else:
                Logger.error(f"不支持的探测类型：{ping_type}")
                return
            await self._report_ping_result(task_id, ping_type, latency)
        except Exception as e:
            Logger.error(f"网络探测异常：{e}")
            await self._report_ping_result(task_id, ping_type, -1)
    
    async def _ping_icmp(self, target: str) -> float:
        """ICMP Ping"""
        if IS_WASM:
            Logger.warning("WASM 环境不支持 ICMP Ping")
            return -1
        try:
            if platform.system() == "Windows":
                command = f"ping -n 1 {target}"
            else:
                command = f"ping -c 1 -W 1 {target}"
            process = await asyncio.create_subprocess_shell(
                command,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await process.communicate()
            output = stdout.decode()
            if platform.system() == "Windows":
                if "时间" in output:
                    import re
                    match = re.search(r'时间 [=<](\d+)ms', output)
                    if match:
                        return float(match.group(1))
            else:
                if "time=" in output:
                    import re
                    match = re.search(r'time=([\d.]+)\s*ms', output)
                    if match:
                        return float(match.group(1))
            return -1
        except Exception:
            return -1
    
    async def _ping_tcp(self, target: str) -> float:
        """TCP Ping"""
        try:
            host, port = target.split(':') if ':' in target else (target, '80')
            port = int(port)
            start_time = time.time()
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(host, port),
                timeout=3.0
            )
            latency = (time.time() - start_time) * 1000
            writer.close()
            await writer.wait_closed()
            return latency
        except Exception:
            return -1
    
    async def _ping_http(self, target: str) -> float:
        """HTTP Ping"""
        try:
            url = target if target.startswith('http') else f"http://{target}"
            start_time = time.time()
            async with self.session.get(url, timeout=5.0) as response:
                latency = (time.time() - start_time) * 1000
                return latency
        except Exception:
            return -1
    
    async def _report_ping_result(self, task_id: str, ping_type: str, value: float):
        """上报网络探测结果"""
        result_data = {
            "type": "ping_result",
            "task_id": int(task_id),
            "ping_type": ping_type,
            "value": value,
            "finished_at": datetime.now().isoformat()
        }
        Logger.debug(f"上报网络探测结果：{json.dumps(result_data, indent=2)}", 2)
    
    async def _handle_terminal(self, event: Dict[str, Any]):
        """处理终端连接"""
        if self.disable_remote_control:
            Logger.warning("远程终端功能已被禁用，忽略请求")
            return
        request_id = event.get('request_id', '')
        if not request_id:
            Logger.error("终端连接请求缺少 request_id")
            return
        Logger.info(f"建立终端连接：{request_id}")
        asyncio.create_task(self._start_terminal_session(request_id))
    
    async def _start_terminal_session(self, request_id: str):
        """启动终端会话"""
        log = lambda msg: Logger.info(f"[终端会话] {msg}")
        log(f"启动终端会话：{request_id}")
        try:
            handler = TerminalSessionHandler()
            await handler.start_session(
                request_id,
                self.config['http_server'],
                self.config['token']
            )
        except Exception as e:
            log(f"启动终端会话失败：{e}")
    
    async def close(self):
        """关闭资源"""
        await self.session.close()
# ============================================================================
# StatusHTTPServerHTTP 状态服务
# ============================================================================       
class StatusHTTPServer:
    """HTTP 状态服务 - 非阻塞异步实现"""
    
    def __init__(self, status: AgentStatus, port: int = 8080):
        self.status = status
        self.port = port
        self.app = None
        self.runner = None
        self.site = None
        
    async def _handle_status(self, request):
        """GET /status - 完整状态"""
        return web.json_response(self.status.to_dict())
    
    async def _handle_health(self, request):
        """GET /health - 健康检查（用于 K8s/负载均衡）"""
        data = self.status.to_dict()
        health = data["health"]["status"]
        status_code = 200 if health == "healthy" else 503
        return web.json_response({"status": health, "timestamp": datetime.now(timezone.utc).isoformat()}, status=status_code)
    
    async def _handle_metrics(self, request):
        """GET /metrics - Prometheus 格式指标"""
        s = self.status
        lines = [
            "# HELP agent_uptime_seconds Agent uptime in seconds",
            "# TYPE agent_uptime_seconds gauge",
            f"agent_uptime_seconds {s.to_dict()['agent']['uptime_seconds'] or 0}",
            "",
            "# HELP agent_errors_total Total error count",
            "# TYPE agent_errors_total counter",
            f"agent_errors_total {s.error_count}",
            "",
            "# HELP agent_websocket_connected WebSocket connection status",
            "# TYPE agent_websocket_connected gauge",
            f"agent_websocket_connected {1 if s.websocket_connected else 0}",
            "",
            "# HELP agent_last_heartbeat_timestamp Last heartbeat timestamp",
            "# TYPE agent_last_heartbeat_timestamp gauge",
            f"agent_last_heartbeat_timestamp {datetime.now().timestamp() if s.last_heartbeat else 0}",
        ]
        return web.Response(text="\n".join(lines), content_type="text/plain; version=0.0.4")
    
    async def _handle_info(self, request):
        """GET /info - 精简基础信息"""
        return web.json_response(self.status.last_basic_info or {"error": "No basic info cached yet"})
    
    async def _handle_root(self, request):
        """GET / - HTML 状态页面"""
        s = self.status.to_dict()
        html = f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>Komari Agent Status</title>
<style>
body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; max-width: 800px; margin: 40px auto; padding: 0 20px; }}
.card {{ border: 1px solid #e0e0e0; border-radius: 8px; padding: 16px; margin: 16px 0; background: #fafafa; }}
.ok {{ color: #2e7d32; }} .warn {{ color: #ed6c02; }} .err {{ color: #c62828; }}
pre {{ background: #f5f5f5; padding: 12px; border-radius: 4px; overflow-x: auto; }}
</style></head><body>
<h1>🔍 Komari Agent Status</h1>
<div class="card">
  <h3>Agent Info</h3>
  <p><strong>Version:</strong> {s['agent']['version']}</p>
  <p><strong>Runtime:</strong> {s['agent']['runtime'].upper()}</p>
  <p><strong>Running:</strong> <span class="{'ok' if s['agent']['is_running'] else 'err'}">{s['agent']['is_running']}</span></p>
  <p><strong>Uptime:</strong> {s['agent']['uptime_seconds'] or 0:.1f} seconds</p>
  <p><strong>Last Heartbeat:</strong> {s['agent']['last_heartbeat'] or 'N/A'}</p>
</div>
<div class="card">
  <h3>Connection</h3>
  <p><strong>WebSocket:</strong> <span class="{'ok' if s['connection']['websocket_connected'] else 'err'}">{s['connection']['websocket_connected']}</span></p>
  <p><strong>Server:</strong> {s['connection']['http_server'] or 'N/A'}</p>
</div>
<div class="card">
  <h3>Health</h3>
  <p><strong>Status:</strong> <span class="{'ok' if s['health']['status']=='healthy' else 'warn' if s['health']['status']=='degraded' else 'err'}">{s['health']['status'].upper()}</span></p>
  <p><strong>Error Count:</strong> {s['health']['error_count']}</p>
  {f'<p><strong>Last Error:</strong> {s["health"]["last_error"]}</p>' if s['health']['last_error'] else ''}
</div>
<div class="card">
  <h3>API Endpoints</h3>
  <ul>
    <li><a href="/status">/status</a> - Full JSON status</li>
    <li><a href="/health">/health</a> - Health check (200/503)</li>
    <li><a href="/metrics">/metrics</a> - Prometheus metrics</li>
    <li><a href="/info">/info</a> - Cached basic info</li>
  </ul>
</div>
</body></html>"""
        return web.Response(text=html, content_type="text/html")
    
    async def start(self):
        """启动 HTTP 服务器"""
        from aiohttp import web
        
        self.app = web.Application()
        self.app.router.add_get('/', self._handle_root)
        self.app.router.add_get('/status', self._handle_status)
        self.app.router.add_get('/health', self._handle_health)
        self.app.router.add_get('/metrics', self._handle_metrics)
        self.app.router.add_get('/info', self._handle_info)
        
        self.runner = web.AppRunner(self.app)
        await self.runner.setup()
        self.site = web.TCPSite(self.runner, '0.0.0.0', self.port)
        await self.site.start()
        Logger.info(f"✅ HTTP 状态服务已启动: http://0.0.0.0:{self.port}")
        
    async def stop(self):
        """停止 HTTP 服务器"""
        if self.runner:
            await self.runner.cleanup()
            Logger.info("🛑 HTTP 状态服务已停止")
# ============================================================================
# 主监控客户端
# ============================================================================
class KomariMonitorClient:
    """主监控客户端"""
    
    def __init__(self, config: Dict[str, Any], status: Optional[AgentStatus] = None):
        self.config = config
        self.disable_remote_control = config.get('disable_remote_control', False)
        
        # ✅ 创建或接收状态对象
        self.status = status or AgentStatus(config)
        if not status:
            self.status.set_config(config)  # 确保 config 已设置
        
        self.system_info = SystemInfoCollector()
        self.event_handler = EventHandler(config, self.disable_remote_control)
        self.last_basic_info_report = 0
        self.BASIC_INFO_INTERVAL = 300
    
    async def run(self):
        """运行监控客户端"""
        # ✅ 更新状态
        self.status.is_running = True
        self.status.start_time = datetime.now(timezone.utc)
        
        Logger.info(f"启动 Komari 监控客户端 (Python 版本)")
        if self.disable_remote_control:
            Logger.info("远程控制功能已禁用")
        
        while True:
            try:
                await self._run_monitoring_cycle()
                self.status.update_heartbeat()  # ✅ 每次循环更新心跳
                await asyncio.sleep(self.config.get('reconnect_interval', 5))
            except Exception as e:
                self.status.record_error(e)  # ✅ 记录错误
                Logger.error(f"监控周期出错：{e}")
                Logger.info(f"{self.config.get('reconnect_interval', 5)}秒后重试...")
                await asyncio.sleep(self.config.get('reconnect_interval', 5))
    
    async def _run_monitoring_cycle(self):
        """运行监控周期"""
        basic_info_url = f"{self.config['http_server']}/api/clients/uploadBasicInfo?token={self.config['token']}"
        ws_url = self.config['http_server'].replace('http', 'ws') + f"/api/clients/report?token={self.config['token']}"
        await self._push_basic_info(basic_info_url)
        await self._start_websocket_monitoring(ws_url, basic_info_url)
    
    async def _push_basic_info(self, url: str) -> bool:
        """推送基础信息"""
        basic_info = await self.system_info.get_basic_info()
        # 🔁 更新状态缓存
        self.status.update_basic_info(basic_info)
        self.status.update_heartbeat()

        Logger.info("基础信息上报数据:")
        Logger.info(json.dumps(basic_info, indent=2))
        print(json.dumps(basic_info, indent=1))
        Logger.debug(f"推送基础信息到：{url}", 1)
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=basic_info) as response:
                    if response.status in (200, 201):
                        Logger.info("基础信息推送成功")
                        self.last_basic_info_report = time.time()
                        return True
                    else:
                        Logger.error(f"基础信息推送失败 - HTTP: {response.status}")
                        return False
        except Exception as e:
            Logger.error(f"基础信息推送异常：{e}")
            return False
    
    async def _start_websocket_monitoring(self, ws_url: str, basic_info_url: str):
        """启动 WebSocket 监控 - 增强重连版"""
        Logger.debug(f"启动 WebSocket 监控: {ws_url}", 2)
        
        retry_count = 0
        max_retries = 20  # 增加重试次数
        
        while retry_count < max_retries:
            try:
                # WASM 环境下使用更保守的连接配置
                connect_kwargs = {
                    'ping_interval': 20,
                    'ping_timeout': 15,
                    'close_timeout': 5,
                }
                if IS_WASM:
                    connect_kwargs['extensions'] = []  # 禁用可能不兼容的扩展
                
                async with websockets.connect(ws_url, **connect_kwargs) as websocket:
                    Logger.info("✅ WebSocket 连接成功，开始监控")
                    self.status.update_websocket_status(True)
                    retry_count = 0  # 连接成功后重置计数
                    
                    # 启动消息处理任务（确保异常被正确传播）
                    message_task = asyncio.create_task(
                        self._handle_websocket_messages(websocket)
                    )
                    monitoring_task = asyncio.create_task(
                        self._monitoring_loop(websocket, basic_info_url)
                    )
                    
                    # 等待任意任务完成
                    done, pending = await asyncio.wait(
                        [message_task, monitoring_task],
                        return_when=asyncio.FIRST_COMPLETED
                    )
                    # 取消未完成的任务
                    for task in pending:
                        task.cancel()
                        try:
                            await task  # 确保取消后的异常被检索
                        except asyncio.CancelledError:
                            pass
                            
            except ConnectionClosedOK:
                Logger.info("🔌 WebSocket 正常关闭，正在重连...")
                self.status.update_websocket_status(False)
                retry_count += 1
                
            except ConnectionClosedError as e:
                Logger.warning(f"⚠️ WebSocket 异常断开: {e}，正在重连...")
                self.status.update_websocket_status(False)
                retry_count += 1
                
            except OSError as e:
                Logger.warning(f"🌐 网络错误: {e}，正在重连...")
                self.status.update_websocket_status(False)
                retry_count += 1
                
            except Exception as e:
                Logger.error(f"❌ WebSocket 监控异常: {type(e).__name__}: {e}")
                self.status.update_websocket_status(False)
                retry_count += 1
            
            # 🔄 指数退避重连
            if retry_count < max_retries:
                delay = min(30, 2 ** retry_count)  # 最大30秒
                Logger.info(f"⏳ {delay}秒后重试连接 ({retry_count}/{max_retries})...")
                await asyncio.sleep(delay)
            else:
                Logger.error("🚫 重连次数耗尽，等待基础信息上报周期后重试")
                await asyncio.sleep(self.BASIC_INFO_INTERVAL)
                retry_count = 0  # 重置，允许新一轮重试
        
        Logger.info("🔄 重新进入监控周期")
    
    async def _handle_websocket_messages(self, websocket):
        """处理 WebSocket 消息 - 增强异常处理版"""
        try:
            async for message in websocket:
                try:
                    if isinstance(message, str):
                        event = json.loads(message)
                        Logger.debug(f"收到服务器消息: {json.dumps(event, indent=2)}", 2)
                        await self.event_handler.handle_event(event)
                    else:
                        Logger.debug(f"收到二进制消息，长度: {len(message)}", 2)
                except json.JSONDecodeError as e:
                    Logger.warning(f"JSON 解析失败: {e}")
                except Exception as e:
                    Logger.error(f"处理消息异常: {type(e).__name__}: {e}")
                    
        except ConnectionClosedOK:
            # ✅ 正常关闭：服务器发送了关闭帧
            Logger.info("🔌 WebSocket 正常关闭 (ConnectionClosedOK)")
            # 不重新抛出，让外层监控循环处理重连
            
        except ConnectionClosedError as e:
            # ⚠️ 异常关闭：如 "no close frame received or sent"
            Logger.warning(f"⚠️ WebSocket 异常断开: {e}")
            # 不重新抛出，让外层监控循环处理重连
            
        except OSError as e:
            # 🌐 网络层错误（WASM 常见）
            Logger.warning(f"🌐 网络层错误: {e}")
            # 不重新抛出
            
        except asyncio.CancelledError:
            # 🔄 任务被取消（正常流程）
            Logger.debug("🔄 WebSocket 消息任务被取消")
            raise  # 重新抛出 CancelledError，让 gather 正常处理
            
        except Exception as e:
            # ❌ 未知异常
            Logger.error(f"❌ WebSocket 消息循环未知异常: {type(e).__name__}: {e}")
            # 记录但不抛出，避免 Task 静默失败
    
    async def _monitoring_loop(self, websocket, basic_info_url: str):
        """监控循环 - 增强状态跟踪版"""
        sequence = 0
        interval = max(0.1, self.config.get('interval', 1.0))
        
        while True:
            start_time = time.time()
            
            try:
                # 检查是否需要上报基础信息
                current_time = time.time()
                if current_time - self.last_basic_info_report >= self.BASIC_INFO_INTERVAL:
                    success = await self._push_basic_info(basic_info_url)
                    if success:
                        self.last_basic_info_report = current_time
                    else:
                        self.last_basic_info_report = current_time - self.BASIC_INFO_INTERVAL + 30
                
                # 获取并发送实时监控数据
                realtime_info = await self.system_info.get_realtime_info()
                
                await websocket.send(json.dumps(realtime_info))
                sequence += 1
                
                # ✅ 发送成功：确认连接正常
                self.status.update_websocket_status(True)
                Logger.debug(f"第 {sequence} 条数据发送成功", 2)
                
            except ConnectionClosed as e:
                # ✅ 连接关闭：更新状态
                self.status.update_websocket_status(False)
                Logger.warning(f"🔌 WebSocket 连接关闭：{e}")
                break
                
            except Exception as e:
                # ✅ 发送失败：更新状态
                self.status.update_websocket_status(False)
                self.status.record_error(e)
                Logger.error(f"发送监控数据失败：{e}")
                break
            
            # 控制发送频率
            elapsed = time.time() - start_time
            sleep_time = max(0, interval - elapsed)
            if sleep_time > 0:
                await asyncio.sleep(sleep_time)

# ============================================================================
# 配置解析 (保持不变)
# ============================================================================
def parse_args() -> Dict[str, Any]:
    """解析命令行参数"""
    args = {
        'http_server': None, 'token': None, 'interval': None,
        'reconnect_interval': None, 'ignore_unsafe_cert': None,
        'log_level': None, 'disable_remote_control': None
    }
    argv = sys.argv[1:]
    i = 0
    while i < len(argv):
        arg = argv[i]
        if arg == '--http-server' and i + 1 < len(argv):
            args['http_server'] = argv[i + 1]
            i += 1
        elif arg == '--token' and i + 1 < len(argv):
            args['token'] = argv[i + 1]
            i += 1
        elif arg == '--interval' and i + 1 < len(argv):
            args['interval'] = float(argv[i + 1])
            i += 1
        elif arg == '--log-level' and i + 1 < len(argv):
            args['log_level'] = int(argv[i + 1])
            i += 1
        elif arg == '--disable-web-ssh':
            args['disable_remote_control'] = True
        elif arg in ('--help', '-h'):
            _show_help()
            sys.exit(0)
        i += 1
    return args

def merge_config(cli_config: dict, env_config: dict) -> dict:
    merged = env_config.copy()
    for key, value in cli_config.items():
        if value is not None:
            merged[key] = value
    return merged

def get_final_config() -> Dict[str, Any]:
    """获取最终配置"""
    cli_config = parse_args()
    env_config = parse_env_args()
    config = merge_config(cli_config, env_config)
    if not config['http_server']:
        print("错误：必须提供 --http-server 参数或设置 KOMARI_HTTP_SERVER 环境变量")
        _show_help()
        sys.exit(1)
    http_server = config.get('http_server', '')
    if isinstance(http_server, str):
        config['http_server'] = http_server.rstrip('/')
    if not config['token']:
        print("错误：必须提供 --token 参数或设置 KOMARI_TOKEN 环境变量")
        _show_help()
        sys.exit(1)
    return config

def _show_help():
    """显示帮助信息"""
    print("komari-agent-python 1.1.0 (WASM Compatible)")
    print()
    print("用法：python agent.py --token <token> [选项]")
    print()
    print("选项:")
    print("  --http-server <url>        服务器地址 (必须)")
    print("  --token <token>            认证令牌 (必须)")
    print("  --interval <sec>           实时数据上报间隔 (默认：1.0 秒)")
    print("  --log-level <level>        日志级别：0-5")
    print("  --disable-web-ssh          禁用远程控制功能")
    print("  --help                     显示此帮助信息")

async def check_environment() -> bool:
    """检查运行环境"""
    print("正在检查运行环境...")
    errors = []
    warnings = []
    
    python_version = sys.version_info
    if python_version < (3, 7):
        errors.append("需要 Python 3.7 或更高版本")
    else:
        print(f"✅ Python 版本：{python_version.major}.{python_version.minor}.{python_version.micro}")
    
    # WASM 环境检查
    if IS_WASM:
        print("✅ 检测到 WASM/WASI 环境")
        print("⚠️  部分功能已自动降级:")
        print("   - psutil: 使用模拟数据")
        print("   - 终端会话：已禁用")
        print("   - 命令执行：已禁用")
        print("   - ICMP Ping: 已禁用")
        return True
    
    # 原生环境检查
    required_modules = [
        ('aiohttp', 'aiohttp'),
        ('websockets', 'websockets'),
        ('psutil', 'psutil')
    ]
    for module_name, package_name in required_modules:
        try:
            __import__(package_name)
            print(f"✅ 模块 {module_name} 可用")
        except ImportError:
            errors.append(f"缺少必要模块：{module_name}，请运行：pip install {package_name}")
    
    if platform.system() != "Windows":
        required_commands = ['ping']
        for cmd in required_commands:
            if shutil.which(cmd) is not None:
                print(f"✅ 系统命令 {cmd} 可用")
            else:
                warnings.append(f"缺少系统命令：{cmd}，部分功能可能受限")
        
        try:
            import pty
            print("✅ PTY 终端支持可用")
        except ImportError:
            warnings.append("PTY 支持不可用，终端功能将受限")
    
    if warnings:
        print("\n⚠️  警告:")
        for warning in warnings:
            print(f"   - {warning}")
    
    if errors:
        print("\n❌ 环境检查失败，发现以下问题:")
        for error in errors:
            print(f"   - {error}")
        return False
    
    print("✅ 环境检查通过，所有依赖项均可用")
    return True

async def main():
    """主函数 - 并发运行监控 + HTTP 状态服务"""
    try:
        config = get_final_config()
        
        # 1. 创建共享状态对象
        status = AgentStatus(config)
        
        # 2. 环境检查
        if not await check_environment():
            sys.exit(1)
            
        Logger.set_log_level(config['log_level'])
        
        # 3. 创建监控客户端（注入状态对象）
        client = KomariMonitorClient(config, status)
        
        # 4. 启动 HTTP 状态服务（默认端口 8080，可通过环境变量覆盖）
        http_port = int(os.getenv('KOMARI_STATUS_PORT', 8080))
        http_server = StatusHTTPServer(status, port=http_port)
        await http_server.start()
        status.http_server = http_server  # 保存引用用于清理
        
        # 5. 🔥 并发运行：监控任务 + HTTP 服务（非阻塞！）
        Logger.info(f"🚀 启动并发任务: 监控循环 + HTTP 状态服务(:{http_port})")
        
        # 方案 A: 使用 gather（两者都是长期运行任务）
        await asyncio.gather(
            client.run(),  # 监控主循环
            _keep_alive(),  # 空任务保持 gather 不退出
            return_exceptions=True
        )
        
    except KeyboardInterrupt:
        Logger.info("👋 程序被用户中断，正在清理...")
    except Exception as e:
        Logger.error(f"💥 程序异常: {type(e).__name__}: {e}")
        sys.exit(1)
    finally:
        # 6. 清理资源
        if 'http_server' in locals() and http_server:
            await http_server.stop()
        if 'client' in locals() and hasattr(client, 'event_handler'):
            await client.event_handler.close()
        Logger.info("🧹 资源清理完成，程序退出")

async def _keep_alive():
    """辅助任务：保持 asyncio.gather 不立即返回"""
    try:
        while True:
            await asyncio.sleep(3600)  # 每小时唤醒一次
    except asyncio.CancelledError:
        pass  # 正常取消

if __name__ == "__main__":
    asyncio.run(main())
